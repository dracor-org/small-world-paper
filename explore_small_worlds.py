...
def filter_corpus(df: pd.DataFrame, corpora: list[str], num_of_segments: tuple[int, int], years: tuple[int, int]) -> pd.DataFrame:
    ...
    selected_corpus = selected_corpus.loc[selected_corpus['numOfSegments'].isin(segment_range)]
    selected_corpus = selected_corpus.loc[selected_corpus['yearNormalized'].isin(year_range)]
...
def get_metadata_param(df: pd.DataFrame, corpus_name: str) -> pd.DataFrame:
    ...
    metadata = {
                ...
                "S_mean": df.S.mean(),
                "swn1_abs": swn_abs,
                "swn1_rel": (swn_abs / num_of_plays) * 100,
                "swt_abs": swt_abs,
...
def get_century_to_parameters_df(df: pd.DataFrame, metrics_of_interest:list[str] = ["swn1", "swt", "sft"]) -> pd.DataFrame:
...
def display_aggregated_parameters_over_time(df):
    century_col = [round_down_to_century(x) if isinstance(x, (np.integer, int)) else x 
                   for x in df.yearNormalized] 
    df["century"] = century_col
    century_to_parameters_df = get_century_to_parameters_df(df)
    century_to_parameters_df = century_to_parameters_df[century_to_parameters_df.index.notnull()]
...
def display_parameter_corpus_cmp_hv(df): 
    ...
    return df.hvplot.scatter(x="yearNormalized", y=["S"], by="swt in corpus", legend="right",
                               hover_cols=["firstAuthor", "title", "name", "corpus_name"], responsive=True)
...
def get_network_from_dracor(corpus_name, play_name):
    ...
    if graphml.ok:
        ...
        print(f"The play {play_name} could not be found in corpus {corpus_name}. Please check both of your inputs.")
    return graphml
...
# Widgets
corpus_widget = pn.widgets.MultiChoice(name="Corpus", value=corpus_names, options=corpus_names)
segment_widget = pn.widgets.RangeSlider(name="Number of segments", value=(1, results.numOfSegments.max()), 
                                      start=1, end=results.numOfSegments.max(), step=1)
year_widget = pn.widgets.RangeSlider(name="Year Normalized", value=(results.yearNormalized.min(), 
                                                                    results.yearNormalized.max()),
                                   start=results.yearNormalized.min(), end=results.yearNormalized.max(),
                                   step=1)

# Bind widget to functions
metadata_bound = pn.bind(get_filtered_metadata, df=results, corpora=corpus_widget, num_of_segments=segment_widget,years=year_widget)
fig_7_bound = pn.bind(get_fig_7, df=results, corpora=corpus_widget, num_of_segments=segment_widget,years=year_widget)
fig_8_bound = pn.bind(get_fig_8, df=results, corpora=corpus_widget, num_of_segments=segment_widget,years=year_widget)
...
template.main[0:2, 0:3] = pn.Column(corpus_widget, segment_widget, year_widget)
template.main[0:2, 3:12] = pn.Row(metadata_bound)
template.main[2:5, 0:12] = pn.Row(fig_7_bound, fig_8_bound)
