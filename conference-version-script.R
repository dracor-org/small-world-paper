...
corpora <- c("als", "bash", "cal", "fre", "ger", "greek", "hun", "ita", "rom", "rus", "span", "swe", "tat", "shake")


## A function to get the network metrics for a play
get_network_data_per_play <- function(play_name, corpus){
  json <- fromJSON(paste0("https://dracor.org/api/corpora/", corpus, "/play/", play_name, "/metrics"))
  nodes_data <- json[["nodes"]]
  nodes_data
}

## Downloading and sorting of names of plays in the corpora
get_network_data <- function(corpus){
  list_of_names <- fromJSON(paste0("https://dracor.org/api/corpora/", corpus))
  ...
  plays_data <- lapply(sorted_names, function(x) get_network_data_per_play(x, corpus))
  df <- data.frame(sorted_ids)
  df$network_metrics <- plays_data
  return(df)
}

network_metrics_df <- lapply(corpora, get_network_data)
vb_metrics <- do.call(rbind, network_metrics_df)


get_metadata <- function(corpus){
  metadata <- read.csv(file=paste0("https://staging.dracor.org/api/corpora/", corpus, "/metadata/csv"), stringsAsFactors = F)
  metadata <- metadata[order(metadata$id),]
  meta_subset <- subset(metadata, select = c(id, name, firstAuthor, title, subtitle, normalizedGenre, numOfSpeakers, yearNormalized, size, numOfSegments, numConnectedComponents, averageClustering, averagePathLength, numEdges))
  return(meta_subset)
}

options(timeout=500) ## It takes a while sometimes to load metadata, so the timeout is increased
metadata <- lapply(corpora, get_metadata)
names(metadata) <- corpora

vb_metadata <- do.call("rbind", metadata)
vb_metadata <- vb_metadata[!vb_metadata$name=="menander-dyskolos",] ## This play was added after the start of working on this project, so we remove it

## Adding network metrics to metadata
vb_metadata$network_metrics <- vb_metrics$network_metrics

vb_metadata <- vb_metadata[!vb_metadata$name=="kraus-die-letzten-tage-der-menschheit", ] ## The giant play with - it is removed to have homogeneous corpus 
vb_metadata <- vb_metadata[!vb_metadata$size==0, ] ## Removing plays with size equals 0
...
randomize_graph <- function(numOfNodes, numEdges){
  random_graphs=list(1000)
  for (i in 1:1000){
    random_graphs[[i]] <- sample_gnm(numOfNodes, numEdges, directed = F, loops = F)
  }
...
## Function for the Small-World-Test (swt) testing two criteria
...
## Analysis 4 - calculaing R2 on VeBiDraCor
...
## Analysis 6 - calculaing R2 on VeBiDraCor_Struc
...
## Analysis 8 - calculaing R2 on VeBiDraCor_Struc_Hist
...
## Analysis 10 - calculaing R2 on GerDraCor
...
## Analysis 12 - calculaing R2 on RusDraCor
...
## Writing the metadata table with analyses results into file
